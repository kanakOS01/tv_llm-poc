...
def compute_mfccs(buffer, sample_rate, fft_size, hop_size, n_mfcc):
    ...
    X = librosa.feature.mfcc(
        y=buffer.data[0], sr=sample_rate, n_fft=fft_size, hop_length=hop_size, n_mfcc=20
    )
...
def extract_audio_features(buffer, Y, fft_size, hop_size):
    ...
    centroid = librosa.feature.spectral_centroid(
        y=buffer.data[0], sr=buffer.sample_rate, n_fft=fft_size, hop_length=hop_size
    )[0]
    flatness = librosa.feature.spectral_flatness(
        y=buffer.data[0], n_fft=fft_size, hop_length=hop_size
    )[0]
...
def analyse_audio(buffer, fft_size=16384, n_mfcc=20, n_components=2):
    ...
    index, timestamp, duration_array = create_data_series(
        Y, sample_rate, fft_size, hop_size
    )
    centroid, flatness, labels = extract_audio_features(buffer, Y, fft_size, hop_size)
    df = create_features_df(
        Y, centroid, flatness, index, timestamp, duration_array, labels
    )
    return df
...
def df_to_np_dict(df):
    return {
        k: np.array(v).astype(np.float32) for k, v in df.to_dict(orient="list").items()
    }
...
def main(**kwargs):
    ...
    tv.s.grains = {
        "state": {
            "x": (ti.f32, -3.0, 3.0),
            "y": (ti.f32, -3.0, 3.0),
            "centroid": (ti.f32, 0.0, 20000.0),
            "flatness": (ti.f32, 0.0, 100.0),
            "timestamp": (ti.f32, 0.0, 1.0),
            "duration": (ti.f32, 0.0, 1.0),
            "cluster": (ti.i32, 0.0, 1.0),
        },
        "shape": (len(df)),
    }
    ...
    def grain_pos_f32(i) -> ti.math.vec2:
        ...
        x, y = ti_map_range(x, -3.0, 3.0, 0.0, tv.x), ti_map_range(
            y, -3.0, 3.0, 0.0, tv.y
        )
