...
def main(**kwargs):
    ...
    pd_bela = kwargs.get(
        "pd_bela", False
    )  # True will also create a `_main.pd` patch for Bela
    ...
    """
    osc_map = OSCMap(
        osc, client_name, patch_type, patch_name, pd_bela=pd_bela, export=export
    )
    ...
    @osc_map.send_list(
        vector=(0.0, 0.0, 100.0), length=10, count=update_rate, send_mode=send_mode
    )
