...
def main(**kwargs):
    ...
    def _(ls):
        nonlocal vel
        vel += ti.Vector(
            [map_range(ls.x, 0, 255, -1, 1), map_range(ls.y, 255, 0, -1, 1)]
        )
    ...
    def update_bullets():
        for i in range(bullets.shape[0]):
            if bullets[i].active == 1:
                ...
                if (
                    bullets[i].pos.x < 0
                    or bullets[i].pos.x > tv.x
                    or bullets[i].pos.y < 0
                    or bullets[i].pos.y > tv.y
                ):
                    bullets[i].active = 0
                tv.px.circle(
                    bullets[i].pos.x,
    ...
    def update(pos: ti.math.vec2, ang: ti.f32):
        ...
        v2 = ti.Vector(
            [pos.x + size * ti.cos(ang + offset), pos.y + size * ti.sin(ang + offset)]
        )
        v3 = ti.Vector(
            [pos.x + size * ti.cos(ang - offset), pos.y + size * ti.sin(ang - offset)]
        )
        tv.px.triangle(v1, v2, v3, ti.Vector([1.0, 1.0, 1.0, 1.0]))
            ...
            if bullets[i].active == 1:
                for j in range(tv.pn):
                    if (bullets[i].pos - tv.p.field[j].pos).norm() < 10:
                        bullets[i].active = 0
                        tv.p.field.vel[j] = ti.Vector([0.0, 0.0])
                        tv.p.field[j].active = 0.0
